...
插件的浏览器入口导出 `apply(ctx)`。它通过 `ctx.effect()` 和 `ctx.slots.inject()` 在插件生命周期内注册两个纯增量贡献——从不声明或替换宿主拥有的 slot。空白会话任务启动器以 `id: 'workbench', order: 10` 注册到 `conversation.input.dock` 列表 slot，活跃任务指示器以 `id: 'workbench', order: 0` 注册到 `conversation.input.right` 列表 slot。两者与已随宿主发布的条目（todo、queue 等）共存，并在插件停用或卸载时一起移除。
...
任务启动器仅在空白且空闲的会话中出现，选择 starter 时通过 `inputActions.setDraft` 填充作曲者草稿。活跃任务指示器展示运行/排队状态与小型进度条。两个组件的 token 和样式都限定在 `data-dsh-developer-workbench="true"` 标记之下。
...
`tests/registration.client.spec.tsx` 锁定两个增量注册（dock + right，含 `id`/`order`）及其销毁行为，并断言插件从不声明宿主拥有的 seat（`root`、`details`、`tool.call.toolview`）。`tests/presentation.client.spec.tsx` 通过注入的 `useSession` selector hook 渲染两个贡献；`tests/browser.workbench.spec.tsx` 把它们真实挂载到 DOM 并驱动 starter 点击进入 `inputActions.setDraft`。套件还通过限定作用域的 CSS contract test 检查 focus-visible 和 reduced-motion 规则。
